...
const DeleteModal = ({ isOpen, setIsOpen, workspace }: Props) => {
    ...
    const { removeWorkspaceFromList } = useWorkspaceContext();
    ...
    const handleDeleteClick = () => {
        ...
        deleteWorkspace.mutate(workspace.uuid, {
            onSuccess: () => {
                removeWorkspaceFromList(workspace.uuid);

                queryClient.setQueryData(
                    ["workspaces"],
                    (old: Workspace[] | undefined) =>
                        old?.filter((w) => w.uuid !== workspace.uuid),
                );
        ...
        <Modal isOpen={isOpen} close={() => setIsOpen(false)}>
            ...
            <Modal.Body>
                ...
                <div className="flex items-center gap-x-2">
                    ...
                    <button
                        className="flex-1 rounded-ld py-1.5 text-lg bg-red-600 rounded-lg text-white/80 font-medium cursor-pointer hover:bg-red-700 transition-[background-color] duration-200 flex items-center justify-center gap-x-1"
